...
const ReviewsSection = ({ tutorId, initialFeedbacks = [] }: ReviewsSectionProps) => {
    ...
    const fetchUserInfo = async () => {
      try {
        ...
        const tutorSlots = allSlots.filter(
          (slot: any) => slot.tutor_id === tutorId && slot.user_id === userId
        );
        ...
        tutorSlots.forEach((slot: any) => {
          const planId = slot.booking_planid;
        ...
        setUserBookings(Array.from(bookingPlansMap.values()));
      } catch (err) {
  ...
  const userFeedbacks = userBookings.filter(b => b.hasFeedback && b.feedbackId);

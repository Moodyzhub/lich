...
export default function CreateCourse() {
  ...
  const handleStep2Save = async (sectionsData: SectionData[]) => {
    ...
    try {
      ...
      // Check if error is 404 (course not found)
      if (err?.response?.status === 404 || err?.message?.includes('404') || err?.message?.includes('không tìm thấy')) {
        // Course was deleted, clear localStorage and reset
        sessionStorage.removeItem(STORAGE_KEY);
        toast({
          variant: "destructive",
          title: "Khóa học không tồn tại",
